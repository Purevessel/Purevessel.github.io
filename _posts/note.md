...
https://yeefea.com/os/glibc_malloc/

#征服C指针#

C语言未对局部变量在栈中的内存分配顺序给出指定标准，因此在理想的无任何限制条件下，定义顺序并不能代表实际在内存中存储的顺序

在表达式中，数组下标运算符"[]"与数组无关系，"int array[10]" 中的array就是一个指针变量。因此，array[4] 和 [4]array实际上是等同的，但显然后者不是给正常人阅读用的

C99标准中允许启用非static局部变量定义的变长数组

malloc与free都并非系统调用，而是glibc库函数。malloc调用了brk()/sbrk()/mmap()这三个系统调用来进行内存申请，其中brk()/sbrk()负责小块的内存分配，mmap()则是负责大块的内存分配。这个“大小”的分界点在分配内存是否小于128kb

free()函数的调用之后，指向的内存并不是立刻就会释放给操作系统。这是因为它管理的是chunk而非实际的内存（*这篇原理我还没弄清楚，先这样描述，存疑*：https://zhuanlan.zhihu.com/p/16029505624）,所以有可能存在程序员认为自己free掉了某个结构体，但是结构体中的数据仍然能访问的场景，这样的BUG排查起来会非常痛苦。作者意识推荐了一款linux专用的调试工具Valgrind，另外一个思路就是包装free，工作中实际调用的free在每次调用后都会尽可能将所期望释放的内存全部置空，达到破坏数据避免访问的目的。
...
` int (*fun_p)(double);`
 ...
 这一串声明，我们就先从fun_p开始，看到左侧的加上括号的*，说明这是一个名为fun_p的指针，右侧带double的括号说明这是一个函数，即指向一个带有double参数的函数指针。最左边的int则说明了这个函数返回int类型。所以，fun_p是一个指向带有都变了参数且返回int类型函数的函数指针

左值与右值可以理解为“表达式代表的是某处的存储空间”与“表达式仅代表值”
